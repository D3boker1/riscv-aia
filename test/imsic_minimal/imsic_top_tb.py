...
async def debug_config(dut):
    ...
    await Timer(ONE_CYCLE, units="ns")
    imsic_write_reg(dut, 1, EITHRESHOLD, 3, M_MODE)
    await Timer(ONE_CYCLE, units="ns")
    imsic_write_reg(dut, 1, EITHRESHOLD, 2, S_MODE)
    await Timer(ONE_CYCLE, units="ns")
...
# NR_VS_FILES_PER_IMSIC = 0
##############################################
async def simple_config_two_imsic(dut):
    imsic_write_reg(dut, 1, EDELIVERY, 1, M_MODE)
    await Timer(ONE_CYCLE, units="ns")
    imsic_write_reg(dut, 1, EDELIVERY, 1, S_MODE)
    await Timer(ONE_CYCLE, units="ns")
    ...
    imsic_stop_write(dut)
    await Timer(ONE_CYCLE, units="ns")
    imsic_write_reg(dut, 2, EDELIVERY, 1, M_MODE)
    await Timer(ONE_CYCLE, units="ns")
    imsic_write_reg(dut, 2, EDELIVERY, 1, S_MODE)
    await Timer(ONE_CYCLE, units="ns")
    imsic_write_reg(dut, 2, EITHRESHOLD, 3, M_MODE)
    await Timer(ONE_CYCLE, units="ns")
    imsic_write_reg(dut, 2, EITHRESHOLD, 2, S_MODE)
    await Timer(ONE_CYCLE, units="ns")
    imsic_stop_write(dut)
...
# NR_VS_FILES_PER_IMSIC = 0
##############################################
async def two_imsic_with_m_s_files(dut):
    ...
    # Simulate a dummy device write to S-File 
    axi_write_reg(dut,0x28001000,30)
    await Timer(ONE_CYCLE, units="ns")
    axi_disable_write(dut)
    await Timer(ONE_CYCLE*4, units="ns")
...
# NR_VS_FILES_PER_IMSIC = 1
##############################################
async def two_imsic_with_vs_files(dut):
    # Enable interrupt delivering for M-File
    imsic_write_reg(dut, 1, EDELIVERY, 1, M_MODE, 0)
    await Timer(ONE_CYCLE, units="ns")
    # Enable interrupt delivering for S-File
    imsic_write_reg(dut, 1, EDELIVERY, 1, S_MODE, 0)
    await Timer(ONE_CYCLE, units="ns")
    # Enable interrupt delivering for VS-File
    imsic_write_reg(dut, 2, EDELIVERY, 1, S_MODE, 1)
    await Timer(ONE_CYCLE, units="ns")

    # Enable interrupt 30 in S-File
    imsic_write_reg(dut, 1, EIE0, 0x40000000, S_MODE, 0)
    await Timer(ONE_CYCLE, units="ns")
    imsic_stop_write(dut)
    await Timer(ONE_CYCLE, units="ns")
    # Enable interrupt 2 in M-File
    imsic_write_reg(dut, 1, EIE0, 0x00000004, M_MODE, 0)
    await Timer(ONE_CYCLE, units="ns")
    imsic_stop_write(dut)
    await Timer(ONE_CYCLE, units="ns")
    # Enable interrupt 13 in VS-File
    imsic_write_reg(dut, 2, EIE0, 0x00002000, S_MODE, 1)
    await Timer(ONE_CYCLE, units="ns")
    imsic_stop_write(dut)
    await Timer(ONE_CYCLE, units="ns")
    ...
    axi_write_reg(dut,0x28003000,13)
    await Timer(ONE_CYCLE, units="ns")
    axi_disable_write(dut)
    
    #clear the pending interrupt
    await Timer(ONE_CYCLE*3, units="ns")
    imsic_write_xtopei(dut, 1, S_MODE, 0)

    #clear the pending interrupt
    await Timer(ONE_CYCLE*3, units="ns")
    imsic_write_xtopei(dut, 1, M_MODE, 0)

    #clear the pending interrupt
    await Timer(ONE_CYCLE*3, units="ns")
    imsic_write_xtopei(dut, 2, S_MODE, 1)
...
# NR_VS_FILES_PER_IMSIC = 1
##############################################
async def four_imsic_with_vs_files(dut):
    FILE_UT_1 = 3
    FILE_UT_2 = 2
    # Enable interrupt delivering for M-File
    ...
    # Simulate a dummy device write 
    axi_write_reg(dut,0x28000000 + (FILE_UT_1-1)*2*0x1000,30)
    await Timer(ONE_CYCLE, units="ns")
    axi_disable_write(dut)
    await Timer(ONE_CYCLE*4, units="ns")
    ...
    axi_write_reg(dut,0x28001000 + (FILE_UT_1-1)*2*0x1000,13)
    await Timer(ONE_CYCLE, units="ns")
    axi_disable_write(dut)
    await Timer(ONE_CYCLE*4, units="ns")
    ...
    axi_write_reg(dut,0x24000000 + (FILE_UT_2-1)*0x1000,2)
    await Timer(ONE_CYCLE, units="ns")
    axi_disable_write(dut)
    await Timer(ONE_CYCLE*4, units="ns")
